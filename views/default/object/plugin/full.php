...
?>
			...
			<?php 
				...
				if(!subsite_manager_on_subsite() && subsite_manager_is_superadmin_logged_in()){
					echo " | ";
					echo elgg_view("output/url", array(
						"href" => "#subsite-manager-plugin-super-admin-options-" .  $plugin->getID(),
						"text" => "&pi;",
						"rel" => "toggle"
					));
				}
	...
	if(!subsite_manager_on_subsite() && subsite_manager_is_superadmin_logged_in()){ 
		...
		if(!subsite_manager_check_global_plugin_setting($plugin->getID(), "enabled_everywhere")){
			echo elgg_view("output/confirmlink", array(
				"href" => "action/plugins/disable_all?plugin=" . $plugin->getID(),
				"text" => elgg_echo("subsite_manager:plugins:advanced:disable_all"),
				"title" => elgg_echo("subsite_manager:plugins:advanced:disable_all:title"),
				"confirm" => elgg_echo("subsite_manager:plugins:advanced:disable_all:confirm")
			));
		}
		echo "</div>";
	} 
