...
function system_log($object, $event) {
	...
	if ($object instanceof Loggable) {
		...
		if (!isset($log_cache[$time][$object_id][$event])) {
			// Subsite Manager: added site guid to creation of eventlog
			$query = "INSERT DELAYED into {$CONFIG->dbprefix}system_log
				(object_id, object_class, object_type, object_subtype, event,
				performed_by_guid, owner_guid, site_guid, access_id, enabled, time_created, ip_address)
			VALUES
				('$object_id','$object_class','$object_type', '$object_subtype', '$event',
				$performed_by, $owner_guid, {$CONFIG->site_guid}, $access_id, '$enabled', '$time', '$ip_address')";
