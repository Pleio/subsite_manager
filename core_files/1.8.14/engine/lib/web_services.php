...
function get_and_validate_api_headers() {
	...
	$result->api_key = elgg_extract("HTTP_X_ELGG_APIKEY", $_SERVER);
	if ($result->api_key == "") {
		throw new APIException(elgg_echo('APIException:MissingAPIKey'));
	}

	$result->hmac = elgg_extract("HTTP_X_ELGG_HMAC", $_SERVER);
	if ($result->hmac == "") {
		throw new APIException(elgg_echo('APIException:MissingHmac'));
	}

	$result->hmac_algo = elgg_extract("HTTP_X_ELGG_HMAC_ALGO", $_SERVER);
	if ($result->hmac_algo == "") {
		throw new APIException(elgg_echo('APIException:MissingHmacAlgo'));
	}

	$result->time = elgg_extract("HTTP_X_ELGG_TIME", $_SERVER);
	if ($result->time == "") {
	...
	$result->nonce = elgg_extract("HTTP_X_ELGG_NONCE", $_SERVER);
	if ($result->nonce == "") {
		throw new APIException(elgg_echo('APIException:MissingNonce'));
	}

	if ($result->method == "POST") {
		$result->posthash = elgg_extract("HTTP_X_ELGG_POSTHASH", $_SERVER);
		if ($result->posthash == "") {
			throw new APIException(elgg_echo('APIException:MissingPOSTHash'));
		}

		$result->posthash_algo = elgg_extract("HTTP_X_ELGG_POSTHASH_ALGO", $_SERVER);
		if ($result->posthash_algo == "") {
			throw new APIException(elgg_echo('APIException:MissingPOSTAlgo'));
		}

		$result->content_type = elgg_extract("CONTENT_TYPE", $_SERVER);
