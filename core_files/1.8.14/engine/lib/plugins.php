...
function elgg_get_max_plugin_priority() {
	...
	$q .= " FROM {$db_prefix}private_settings ps";
	$q .= " JOIN {$db_prefix}entities e ON ps.entity_guid = e.guid";
	$q .= " WHERE ps.name = '{$priority}'";
	$q .= " AND (e.type = 'object' AND e.subtype = {$plugin_subtype_id})";
	$q .= " AND e.site_guid = {$site->getGUID()}";
