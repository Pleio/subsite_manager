...
$owner_guid = 0, $access_id = ACCESS_PRIVATE) {
	...
	if (elgg_trigger_event('annotate', $entity->type, $entity)) {
		// set correct site_guid
		$site_guid = $entity->site_guid;
		
		// If ok then add it
		$result = insert_data("INSERT into {$CONFIG->dbprefix}annotations
			(entity_guid, name_id, value_id, value_type, owner_guid, time_created, access_id, site_guid) VALUES
			($entity_guid,'$name',$value,'$value_type', $owner_guid, $time, $access_id, $site_guid)");
