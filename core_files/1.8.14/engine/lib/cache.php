...
function elgg_get_system_cache() {
	...
	if (!$FILE_PATH_CACHE) {
		$FILE_PATH_CACHE = new ElggFileCache($CONFIG->dataroot . 'system_cache/' . elgg_get_site_entity()->getGUID() . "/");
	}
...
function elgg_regenerate_simplecache($viewtype = NULL) {
	...
	if (!file_exists($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID())) {
		mkdir($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID());
	}
	...
	foreach ($viewtypes as $viewtype) {
		...
		foreach ($CONFIG->views->simplecache as $view) {
			...
			if ($handle = fopen($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID() . "/" . $viewname, 'w')) {
...
function elgg_disable_simplecache() {
	...
	if ($CONFIG->simplecache_enabled) {
		...
		// purge simple cache
		if ($handle = opendir($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID())) {
			while (false !== ($file = readdir($handle))) {
				if ($file != "." && $file != "..") {
					unlink($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID() . "/" . $file);
				}
			}
			closedir($handle);
		}
...
function elgg_invalidate_simplecache() {
	...
	$handle = opendir($CONFIG->dataroot . 'views_simplecache/' . $site->getGUID());
	...
	while (false !== ($file = readdir($handle))) {
		if ($file != "." && $file != "..") {
			$return &= unlink($CONFIG->dataroot . 'views_simplecache/'. $site->getGUID() . "/" . $file);
		}
	}
	closedir($handle);
