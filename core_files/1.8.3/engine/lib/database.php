...
/**
 *  Subsite manager db query time logger
 */
global $SUBSITE_MANAGER_DB_TOTAL_TIME;
$SUBSITE_MANAGER_DB_TOTAL_TIME = 0;
...
function execute_query($query, $dblink) {
	global $CONFIG, $dbcalls, $SUBSITE_MANAGER_DB_TOTAL_TIME, $COLDTRICK_DB_LOG;
	...
	// log the runtime of the query
	$debug_start = microtime(true);
	
	
	
	$result = mysql_query($query, $dblink);
	$query_time = (microtime(true) - $debug_start);
	
	if(elgg_is_admin_logged_in()){
		if(elgg_get_logged_in_user_entity()->username == "jdalsem"){	
			if(!isset($COLDTRICK_DB_LOG)){
				$COLDTRICK_DB_LOG = array();
			}
			...
			if (isset($traces[2]))
			{
				$loc .= $traces[2]['function'] . " - ";
			}
				
			if (isset($traces[3]))
			{
				$loc .= $traces[3]['function'] . " - ";
			}
				
			if (isset($traces[4]))
			{
				$loc .= $traces[4]['function'] . " - ";
			}
	
				
			if (isset($traces[5]))
			{
				$loc .= $traces[5]['function'] . " - ";
			}
	
			if (isset($traces[6]))
			{
				$loc .= $traces[6]['function'] . " - ";
			}
			if (isset($traces[7]))
			{
				$loc .= $traces[7]['function'] . " - ";
			}
				
			if (isset($traces[8]))
			{
				$loc .= $traces[8]['function'] . " - ";
			}
			if (isset($traces[9]))
			{
				$loc .= $traces[9]['function'] . " - ";
			}
			if (isset($traces[10]))
			{
				$loc .= $traces[10]['function'] . " - ";
			}
// 			if (isset($traces[11]))
// 			{
// 				$loc .= $traces[11]['function'] . " - ";
// 			}
// 			if (isset($traces[12]))
// 			{
// 				$loc .= $traces[12]['function'] . " - ";
// 			}
// 			if (isset($traces[13]))
// 			{
// 				$loc .= $traces[13]['function'] . " - ";
// 			}
// 			if (isset($traces[14]))
// 			{
// 				$loc .= $traces[14]['function'] . " - ";
// 			}
// 			if (isset($traces[15]))
// 			{
// 				$loc .= $traces[15]['function'] . " - ";
// 			}
	
			$COLDTRICK_DB_LOG[] = $query_time . " - ". $loc . $query;
		}
	}
	
	// save to runtime of the query
	$SUBSITE_MANAGER_DB_TOTAL_TIME += $query_time;
...
function elgg_row_to_array($row){
	return (array) $row;
}
