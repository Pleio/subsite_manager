...
// $SUBTYPE_CACHE = NULL;
global $SUBTYPE_ID_CACHE;
$SUBTYPE_ID_CACHE = NULL;

global $TYPE_SUBTYPE_CACHE;
$TYPE_SUBTYPE_CACHE = NULL;
...
function get_subtype_id($type, $subtype) {
	global $CONFIG, $TYPE_SUBTYPE_CACHE;
	...
	if(!isset($TYPE_SUBTYPE_CACHE)){
		// load cache
		elgg_load_type_subtype_cache();
	}

	// check if found in cache
	if(isset($TYPE_SUBTYPE_CACHE[$type]) && isset($TYPE_SUBTYPE_CACHE[$type][$subtype])){
		return $TYPE_SUBTYPE_CACHE[$type][$subtype]->id;
	}
...
function get_subtype_from_id($subtype_id) {
	global $CONFIG, $SUBTYPE_ID_CACHE;
	...
	// check if cache is loaded
	if(!isset($SUBTYPE_ID_CACHE)){
		// load cache
		elgg_load_type_subtype_cache();
	}
	
	// check if found in cache
	if(isset($SUBTYPE_ID_CACHE[$subtype_id])){
		return $SUBTYPE_ID_CACHE[$subtype_id]->subtype;
	}
...
function get_subtype_class($type, $subtype) {
	...
	// check if found in cache
	if(isset($TYPE_SUBTYPE_CACHE[$type]) && isset($TYPE_SUBTYPE_CACHE[$type][$subtype])){
		return $TYPE_SUBTYPE_CACHE[$type][$subtype]->class;
	}
...
function get_subtype_class_from_id($subtype_id) {
	...
	// check if found in cache
	if(isset($SUBTYPE_ID_CACHE[$subtype_id])){
		return $SUBTYPE_ID_CACHE[$subtype_id]->class;
	}
...
function add_subtype($type, $subtype, $class = "") {
	...
	if ($id == 0) {
		// invalidate cache
		elgg_invalidate_type_subtype_cache();
...
/**
* 2012-09-24: ColdTrick added type-subtype caching functions
*/

function elgg_load_type_subtype_cache(){
	global $SUBTYPE_ID_CACHE, $TYPE_SUBTYPE_CACHE;

	$query = "SELECT *";
	$query .= " FROM " . elgg_get_config("dbprefix") . "entity_subtypes";

	if($data = get_data($query)){
		$SUBTYPE_ID_CACHE = array();
		$TYPE_SUBTYPE_CACHE = array();

		foreach($data as $row){
			$SUBTYPE_ID_CACHE[$row->id] = $row;
				
			if(!isset($TYPE_SUBTYPE_CACHE[$row->type])){
				$TYPE_SUBTYPE_CACHE[$row->type] = array();
			}
				
			$TYPE_SUBTYPE_CACHE[$row->type][$row->subtype] = $row;
		}
	}
}

function elgg_invalidate_type_subtype_cache(){
	global $SUBTYPE_ID_CACHE, $TYPE_SUBTYPE_CACHE;
 ...
 *
 *	subsite manager: fixes a security bug with memcache
 *
 ...
 *
 *	Subsite manger: added hook to modify $options
 *
...
function get_entity_url($entity_guid) {
	...
	if ($entity = get_entity($entity_guid)) {
		...
		// SUBSITE MANAGER 2.0: trigger hook so the url points to the correct site for entities shown not on entity->site_guid
		$url = elgg_trigger_plugin_hook("entities:get_url", $entity->getType(), array("entity" => $entity), $url);
