...
function register_translations($path, $load_all = false) {
	...
	if(!isset($cache_loaded) && isset($CONFIG->dataroot)){
		$cache_loaded = false;
		
		$site = elgg_get_site_entity();
		$base_path = $CONFIG->dataroot . "language_cache/" . $site->getGUID() . "/";
		$loaded = 0;
		
		if($handler = opendir($base_path)){
		...
		if(!empty($loaded)){
			...
			register_shutdown_function("elgg_cache_languages", $load_language_files);
		}
	}
	
	// couldn't load from cache load normal
	if(!$cache_loaded){
		$handle = opendir($path);
		if (!$handle) {
...
function elgg_languages_init() {
	...
	elgg_register_event_handler("upgrade", "system", "elgg_clear_language_cache");
	
	elgg_register_plugin_hook_handler("action", "admin/plugins/activate", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "admin/plugins/deactivate", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "admin/plugins/activate_all", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "admin/plugins/deactivate_all", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "admin/plugins/set_priority", "elgg_clear_language_cache");
	
	elgg_register_event_handler("language:merge", "translation_editor", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "translation_editor/disable_languages", "elgg_clear_language_cache");
	elgg_register_plugin_hook_handler("action", "translation_editor/delete_language", "elgg_clear_language_cache");
}
...
/**
* 2012-09-24: ColdTrick add language caching
*
*/
function elgg_cache_languages(){
	$site = elgg_get_site_entity();

	$base_path = elgg_get_config("dataroot") . "language_cache/" . $site->getGUID() . "/";

	if(!file_exists($base_path)){
		mkdir($base_path, 0777, true);
	}

	if($translations = elgg_get_config("translations")){

		foreach($translations as $language => $translation){
				
			$content = "<?php" . PHP_EOL;
			$content .= "\$" . $language . " = " . var_export($translation, true) . ";" . PHP_EOL;
			$content .= "add_translation('" . $language . "', \$" . $language . ");" . PHP_EOL;
				
			file_put_contents($base_path . $language . ".php", $content);
		}
	}
}

function elgg_clear_language_cache($site_guid = 0){
