...
// SUBSITE MANAGER 2.0: changed function to support site_guids
function create_metadata($entity_guid, $name, $value, $value_type = '', $owner_guid = 0,
	$access_id = ACCESS_PRIVATE, $allow_multiple = false, $site_guid = 0) {
	...
	// Subsite manager 2.0 set metadata on correct site
	if($site_guid == 0){
		$site_guid = $CONFIG->site_guid;
	}
	$site_guid = elgg_trigger_plugin_hook("create", "metadata", array("entity_guid" => $entity_guid, "metadata_name" => $name, "site_guid" => $site_guid), $site_guid);
	...
	$query = "SELECT * from {$CONFIG->dbprefix}metadata"
		. " WHERE entity_guid = $entity_guid and name_id=" . add_metastring($name) . " and (site_guid = $site_guid OR site_guid IS NULL OR site_guid = 0) limit 1";

	$existing = get_data_row($query);
	if ($existing && !$allow_multiple) {
		$id = (int)$existing->id;
		$result = update_metadata($id, $name, $value, $value_type, $owner_guid, $access_id, $site_guid);
		...
		$query = "INSERT into {$CONFIG->dbprefix}metadata"
			. " (entity_guid, name_id, value_id, value_type, owner_guid, time_created, access_id, site_guid)"
			. " VALUES ($entity_guid, '$name','$value','$value_type', $owner_guid, $time, $access_id, $site_guid)";
...
// SUBSITE MANAGER 2.0: changed function to support site_guids
function update_metadata($id, $name, $value, $value_type, $owner_guid, $access_id, $site_guid = 0) {
	global $CONFIG;
...
$access_id = ACCESS_PRIVATE, $allow_multiple = false, $site_guid = 0) {
	...
	foreach ($name_and_values as $k => $v) {
		$result = create_metadata($entity_guid, $k, $v, $value_type, $owner_guid,
			$access_id, $allow_multiple, $site_guid);
		if (!$result) {
			return false;
		}
	}
